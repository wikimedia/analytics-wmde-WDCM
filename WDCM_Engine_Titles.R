...
publicDataDir <- '/srv/published-datasets/wdcm/WDCM_Titles'
localDataDir <- paste0(fPath, 'WDCM_TitlesData')
...
print("DONE w. titlesData production.")

### --- clear localDataDir
lfs <- list.files()
lfs <- setdiff(lfs, 'titlesData.tsv')
file.remove(lfs)

### --- load and annotate siteLinksData
siteLinksData <- fread('titlesData.tsv', 
                       sep = "\t",
                       quote = "")
...
write.csv(siteLinksData, "titlesData_Processed.csv")
# - clear: siteLinksData
rm(siteLinksData); gc()
...
for (i in 1:length(lFitemtopic)) {
  ...
  if (class(classTopic) == "numeric") {
    classTopic <- as.matrix(t(classTopic))
    rownames(classTopic) <- rownames(topicAnnotation)
  }
...
for (i in 1:length(lF)) {
  ...
  topClassesTopic <- lapply(topClassesTopic, function(x) {
    unlist(x)
  })
...
# - COPY to /srv/published-datasets/wdcm/WDCM_Titles
# - toReport
print("Copying files to: /srv/published-datasets/wdcm/WDCM_Titles")
system(command = 'cp /home/goransm/RScripts/WDCM_R/WDCM_TitlesData/* /srv/published-datasets/wdcm/WDCM_Titles/', 
       wait = T)

# - update string
write(paste0("Last updated on: ", Sys.time()), "wikipediaTitlesUpdateString.txt")
# - copy update string to: /srv/published-datasets/wdcm/WDCM_Titles/
system(command = 'cp /home/goransm/RScripts/WDCM_R/WDCM_TitlesData/wikipediaTitlesUpdateString.txt /srv/published-datasets/wdcm/WDCM_Titles/', 
       wait = T)
